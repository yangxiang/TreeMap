...
enum construction_type {ADJ_GRAPH=0, MATRIX_GRAPH=1, TEDI_ZERO=2, TEDI_ONE=3, SNAP=4};
...
class Graph {
	private:
		...
		void readGraphSNAP(istream&, const graph_type&);//remapping vertices to IDs starting from 0; vertex ID should be nonnegative.
	...
	public:
		vector<int> vertexIDMap;
		vector<int> vertexIDReverseMap;
		Graph();
		Graph(int);
		...
		void writeGraph(ostream&, const graph_type&);
		void writeGraphTediOne(ostream&);//Tedi always treat graphs as undirected.
		void printGraph(const graph_type&);
		int num_vertices();
		int num_edges();
		...
		int get_degree(int);
		int get_directed_degree(int);
		bool hasEdge(int, int);	
		Graph& operator=(const Graph&);
		Vertex& operator[](const int);
		void clear();
		void clearVertex(const int&);
		//void clearVertices(const graph_type&);
...
inline
int Graph::get_directed_degree(int src) {
	return graph_direct[src].inList.size()+graph_direct[src].outList.size();
}
