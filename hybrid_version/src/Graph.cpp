...
Graph::Graph(istream& in, const construction_type& graph_format, const graph_type& direct_info) {
	...
	else if (graph_format==SNAP)
		readGraphSNAP(in, direct_info);
	else
...
void Graph::printGraph(const graph_type& direct_info) {
	writeGraph(cout, direct_info);
}
...
void Graph::clearVertex(const int& v) {
		vector<int>::iterator erase_iter;//erase the edge go to the center point
		
		for(erase_iter=graph[v].begin();erase_iter!=graph[v].end();erase_iter++)
		{
			vector<int>::iterator erase_it=find(graph[*erase_iter].begin(), graph[*erase_iter].end(), v);
			if (erase_it==graph[*erase_iter].end())
				cout<<"Error in decentralizing"<<endl;
			else
			{
				graph[*erase_iter].erase(erase_it);
			}
		}
		graph[v].clear();
		
		if (int(graph_direct.size())<=v)
			return;//It implies the graph is not a valid directed graph.
		
		for(erase_iter=graph_direct[v].outList.begin();erase_iter!=graph_direct[v].outList.end();erase_iter++)
		{
			vector<int>::iterator erase_it=find(graph_direct[*erase_iter].inList.begin(), graph_direct[*erase_iter].inList.end(), v);
			if (erase_it==graph_direct[*erase_iter].inList.end())
				cout<<"Error in decentralizing"<<endl;
			else
			{
				graph_direct[*erase_iter].inList.erase(erase_it);
			}
		}
		graph_direct[v].outList.clear();
		
		for(erase_iter=graph_direct[v].inList.begin();erase_iter!=graph_direct[v].inList.end();erase_iter++)
		{

			vector<int>::iterator erase_it=find(graph_direct[*erase_iter].outList.begin(), graph_direct[*erase_iter].outList.end(), v);
			if (erase_it==graph_direct[*erase_iter].outList.end())
				cout<<"Error in decentralizing"<<endl;
			else
			{
				graph_direct[*erase_iter].outList.erase(erase_it);
			}
		}
		graph_direct[v].inList.clear();
}
...
void Graph::readGraphSNAP(istream& in, const graph_type& direct_info){//vertex ID should be nonnegative
	int n;
	string buf;
	string sub;
	int sid = 0;
	int tid = 0;
	set<int> vertexOriginalIDs;
	while (getline(in, buf)) {
		size_t found= buf.find(" ");
		if(found==string::npos)
			found=buf.find("	");
		sub = buf.substr(0, found);
		istringstream(sub) >> sid;
		vertexOriginalIDs.insert(sid);
		buf.erase(0, found+1);
		istringstream(buf) >>tid;
		vertexOriginalIDs.insert(tid);
	}
	vertexIDMap.clear();
	vertexIDReverseMap.clear();
	vertexIDReverseMap.resize(*(vertexOriginalIDs.rbegin()), 0);
	for(set<int>::iterator sit=vertexOriginalIDs.begin(); sit!=vertexOriginalIDs.end(); sit++){
		vertexIDMap.push_back(*sit);
		vertexIDReverseMap[*sit]=vertexIDMap.size()-1;
	}
	in.clear();
	in.seekg(0, in.beg);
	n=vertexIDMap.size();
	...
	while (getline(in, buf)) {
		...
		buf.erase(0, found+1);
		istringstream(buf) >>tid;
		addEdge(vertexIDReverseMap[sid], vertexIDReverseMap[tid]);//start from 1
		addEdge(vertexIDReverseMap[tid], vertexIDReverseMap[sid]);//start from 1
		if(direct_info==DIRECT){
			addEdgeDirect(vertexIDReverseMap[sid], vertexIDReverseMap[tid]);
		}
	}
}
...
void Graph::writeGraph(ostream& out, const graph_type& direct_info) {
	...
	size_t i = 0;
	if(direct_info==DIRECT){//Output the direct version 
		for (i = 0; i < vl.size(); i++) {
			out << i << ": ";
	...
	}else{//output the undirect version if the direct version 
		for (i = 0; i < vl.size(); i++) {
			out << i << ": ";
			el = graph[i];
			for (eit = el.begin(); eit != el.end(); eit++)
...
void Graph::writeGraphTediOne(ostream& out){//Tedi always treat graphs as undirected.
	out << vl.size() << endl;
	GRA::iterator git;
	EdgeList el;
	EdgeList::iterator eit;
	size_t i = 0;
	//output the undirect version if the direct version 
	for (i = 0; i < vl.size(); i++) {
		el = graph[i];
		for (eit = el.begin(); eit != el.end(); eit++)
			out<<i+1<<" "<<(*eit)+1<<endl;
