...
//		cout<<"Query type: Direct"<<endl;
		gettimeofday(&before_time,NULL);
		for(sit = src.begin(), tit = trg.begin(); 		sit != src.end(); ++sit, ++tit){
			bibfs_distance=GraphUtil::BiBFSDistanceDirect(g, *sit, *tit);
//			bfs_distance=GraphUtil::BFSDistanceDirect(g, *sit, *tit);
//			if(bibfs_distance!=bfs_distance)
//				cout<<"bibfs_distance: "<<bibfs_distance<<"bfs_distance: "<<bfs_distance<<endl;
		}
		gettimeofday(&after_time,NULL);
		float query_time=(after_time.tv_sec - before_time.tv_sec)*1000.0 + (after_time.tv_usec - before_time.tv_usec)*1.0/1000.0;
		cout<<"bibfs query time: "<<query_time<<endl;	
	
	}else{
//		cout<<"Query type: Undirect"<<endl;
		gettimeofday(&before_time,NULL);
		for(sit = src.begin(), tit = trg.begin(); 		sit != src.end(); ++sit, ++tit){
			bibfs_distance=GraphUtil::BiBFSDistance(g, *sit, *tit);
...
//				cout<<"bibfs_distance: "<<bibfs_distance<<"bfs_distance: "<<bfs_distance<<endl;
		...
		gettimeofday(&after_time,NULL);
		float query_time=(after_time.tv_sec - before_time.tv_sec)*1000.0 + (after_time.tv_usec - before_time.tv_usec)*1.0/1000.0;
		cout<<"bibfs query time: "<<query_time<<" (ms)"<<endl;
	}
	cout<<"End query..."<<endl;
