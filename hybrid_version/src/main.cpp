...
inline
bool queryReachabilityDLS(vector<vector<VertexDistancePair> >& LabelDLS_From, vector<vector<VertexDistancePair> >& LabelDLS_To, const int& source_vertex, const int& destination_vertex){
...
static void usage() {
	cout << "\nUsage:\n"
		"	TreeMapF [-h] [-g graph_input_format] [-d direct_info] [-t query_type] [-l rw_queryfile] [-q query_filename] [-n query_num] [-v verify_enable] [-b BAG_SIZE_LIMIT] [-c max_degree_reduction_cutoff] [-s output_LabelDLS] [-s1 output_LabelDLS_From] [-s2 output_LabelDLS_To] [-s3 output_graph] [-ld input_LabelDLS] [-ld1 input_LabelDLS_From] [-ld2 input_LabelDLS_To] [-ld3 input_graph] input_filename\n"
		"Description:\n"
		"	-h	Print the help message.\n"
		"	-g	graph_input_format. default value 0 (adjacent list); range graph_input_format={0,1,2,3,4}.\n"
...
//			destination_vertex = atoi(argv[input_para_counter++]);
	...
	//optional Data structure for DLS involved query on undirected graphs: LabelDLS

	//Data structure must be used for distance query on directed graphs: LabelGT_From_Update, LabelGT_To_Update;
	//optional Data structure for DLS involved query on undirected graphs: LabelDLS_From, LabelDLS_To;
	...
	if(query_type==2)
		cout<<"Total indexing size: "<<TC_size+DLS_size+vertex_mapping_table.size()*2+BitLabelRealNodeLevel.size()+scc_map.size()+1<<endl;	
	else
		cout<<"Total indexing size: "<<TC_size+DLS_size+vertex_mapping_table.size()*2+BitLabelRealNodeLevel.size()+1<<endl;

	cout<<"Start query..."<<endl;	
	struct timeval after_time, before_time;
	int twd_distance=MAX_VAL; 
	int bfs_distance=MAX_VAL;
	bool twd_reachable=false;
	bool bfs_reachable=false;
	...
	if(query_type==1){
			...
			for(sit = src.begin(), tit = trg.begin(); 		sit != src.end(); ++sit, ++tit){
				...
				if((twd_reachable==false) && (Direct_DLS_Activated==true))
					twd_reachable=queryReachabilityDLS(LabelDLS_From, LabelDLS_To, *sit, *tit);
			}
			...
			for(sit = src.begin(), tit = trg.begin(); 		sit != src.end(); ++sit, ++tit){
				...
				if((twd_reachable==false) && (Direct_DLS_Activated==true))
					twd_reachable=queryReachabilityDLS(LabelDLS_From, LabelDLS_To, scc_map[*sit], scc_map[*tit]);
			}
			gettimeofday(&after_time,NULL);
			float query_time=(after_time.tv_sec - before_time.tv_sec)*1000.0 + (after_time.tv_usec - before_time.tv_usec)*1.0/1000.0;
			cout<<"twd reachability query time: "<<query_time<<endl;	
	...
	if(verify_enable==1){
		...
		if(query_type==1){
			for(sit = src.begin(), tit = trg.begin(); 		sit != src.end(); ++sit, ++tit){
				...
				if((twd_reachable==false) && (Direct_DLS_Activated==true))
					twd_reachable=queryReachabilityDLS(LabelDLS_From, LabelDLS_To, *sit, *tit);
				if(GraphUtil::BFSDistanceDirect(g_orig, *sit, *tit)<MAX_VAL)
					bfs_reachable=true;
				else
					bfs_reachable=false;
			...
			for(sit = src.begin(), tit = trg.begin(); 		sit != src.end(); ++sit, ++tit){
				...
				if((twd_reachable==false) && (Direct_DLS_Activated==true))
					twd_reachable=queryReachabilityDLS(LabelDLS_From, LabelDLS_To, scc_map[*sit], scc_map[*tit]);
				if(GraphUtil::BFSDistanceDirect(g_orig, scc_map[*sit], scc_map[*tit])<MAX_VAL)
					bfs_reachable=true;
				else
					bfs_reachable=false;
				if(twd_reachable!=bfs_reachable){
